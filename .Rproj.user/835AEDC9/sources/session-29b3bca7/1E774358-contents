...
PEPRMT_CH4_FINAL <- function(theta,
  ...
  for(i in 1:length(q)){
    ...
    S1_2 = unlist(d[,17]) #Modeled SOC pool - use output from PEPRMT Reco model -cumulative grams C m-3
    S2_2 = unlist(d[,18]) #Modeled labile C pool - use output from PEPRMT Reco model -cumulative grams C m-3
